...
from surrol.algorithms import TD3_WITH_PRIOR
...
if __name__ == '__main__':
    ...
    env = make_vec_env(env_id,1,seed,monitor_dir=log_dir,env_kwargs={'render_mode':'human'})

    prior = TD3.load("./logs/TD3/NeedleGrasp-v0/TD3_HER_NeedleGrasp-v0_200000_steps.zip", env=env, kwargs={})

    n_actions = env.action_space.shape[-1]

    action_noise = NormalActionNoise(mean=np.zeros(n_actions), sigma=0.1 * np.ones(n_actions))

    model = TD3_WITH_PRIOR(policy='MultiInputPolicy', 
        env=env,
        prior=prior,
